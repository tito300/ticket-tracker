...
const TasksList = {
   ...
   renderTasksList() {
    ...
    ulCompleted.classList.add('list-group');
    ul.classList.add('mb-4');

    this.list.sort((a, b) => a.priority - b.priority);

    let deploymentTickets = this.list.filter(i => i.deployment && !i.completed);
    let completedTickets = this.list.filter(i => i.completed);
    let otherTickets = this.list.filter(i => !i.deployment);

    this.list = [...deploymentTickets, ...otherTickets, ...completedTickets];
    ...
    this.list.forEach(task => {
        if(typeof task === 'object') {
            ...
            if(task.deployment) {
                ...
                if(task.completed) {
                    paperPlane.className = 'paper-plane--completed'
